		...
		<div class="comment-body">
			<header class="comment-header">
				<p <?php echo bizznis_attr( 'comment-author' ); ?>>
					...
					printf( '<span itemprop="name">%s</span> <span class="says">%s</span>', $author, apply_filters( 'comment_author_says_text', __( 'says', 'bizznis' ) ) );
					?>
				</p>
				<p class="comment-meta">
					<?php
					$pattern = apply_filters( 'bizznis_comment_time', '<time itemprop="commentTime" datetime="%s"><a href="%s" itemprop="url">%s %s %s</a></time>', $comment );
					printf( $pattern, esc_attr( get_comment_time( 'c' ) ), esc_url( get_comment_link( $comment->comment_ID ) ), esc_html( get_comment_date() ), __( 'at', 'bizznis' ), esc_html( get_comment_time() ) );
			...
			<div class="comment-content" itemprop="commentText">
				<?php if ( $comment->comment_approved == '0' ) { ?>
					<p class="alert"><?php echo apply_filters( 'bizznis_comment_awaiting_moderation', __( 'Your comment is awaiting moderation.', 'bizznis' ) ); ?></p>
				<?php } ?>
				<?php comment_text(); ?>
			</div>
			<?php
