...
/*global bizznis_confirm, confirm, jQuery, bizznis, bizznis_toggles, bizznisL10n */
...
window['bizznis'] = {
	...
	toggle_settings_init: function () {
		...
		jQuery.each(bizznis_toggles, function (k, v) {
			...
			jQuery('div.bizznis-metaboxes').on('change.bizznis.bizznis_toggle', v[0], data, bizznis.toggle_settings);

			// Trigger the check when page loads too.
			// Can't use triggerHandler here, as that doesn't bubble the event up to div.bizznis-metaboxes.
	...
	/**
	 * Have all form fields in Bizznis metaboxes set a dirty flag when changed.
	 *
	 * @since 1.0.0
	 */
	attachUnsavedChangesListener: function () {
		...
		jQuery('div.bizznis-metaboxes :input').change(function(){
		...
		jQuery('div.bizznis-metaboxes input[type="submit"]').click(function(){
